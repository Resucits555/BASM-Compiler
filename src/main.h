...
extern bool hasAux(SymbolData* sym);

extern SymbolScopeCount CountSymbols();
extern char* FindSymbols(const SymbolScopeCount& symbolCount);
extern void CompileSource(IMAGE_SECTION_HEADER(&sections)[]);
extern void WriteSymbolTable(const fs::path srcPath, IMAGE_SECTION_HEADER(&sections)[], char* symtab, const SymbolScopeCount& symbolCount);
